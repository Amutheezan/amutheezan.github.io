...
1.  First install ```XAMPP```/```WAMP```. Then open its control-panel and start ```Apache``` and ```MySQL``` servers. Thereafter, click the admin button and goto       ```PHPMyAdmin``` and create the database to store the data.
...
3.  Install arduino and write the program to read values from sensor and print the values using ```serial.Println(readData)```.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTExMDM4MzAxMTgsMjA1NDA0MzU1NSwtMT
EwMzgzMDExOF19
-->