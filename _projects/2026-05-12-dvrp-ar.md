...
<pre class="mermaid">
{% raw %}flowchart LR
    NR["New Ride Request"] --> ENV(["Environment&lt;br/&gt;(call center)"])
    ENV -- "1. Interrupt anytime solver" --> SOLV{{"Anytime VRP Solver&lt;br/&gt;(DRL Agent)"}}
    SOLV -- "2. Store improved solution" --> DB[("Requests &amp; Routes")]
    ENV -- "3. Send new request" --> INS(["Exhaustive Search Insertion&lt;br/&gt;(DRL Agent)"])
    DB -- "4. Load latest solution" --> INS
    INS -- "5. Accept / reject decision" --> ENV
    ENV -- "6. Add request if accepted" --> DB
