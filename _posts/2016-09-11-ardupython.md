...
1. Install ```pyserial```, you can also download it
...
2. Install ```MYSQLdb```, you can also download it
   from [https://pypi.python.org/pypi/MySQL-python/1.2.5#downloads](https://pypi.python.org/pypi/MySQL-python/1.2.5#downloads).
   Download the ".exe" file and just run it.

3. Then, create the required python program to update the database. For this, you need to add two
   import ```import serial``` and ```import MysqlDb```.

4. Install Arduino and write the program to read values from the sensor and print the values
