...
Each search query is uniquely identified by a UUID. Each node maintains a history of the search queries that have already
arrived at that node, with respect to the UUID. If an incoming search query is not in the history, then it will either
find it on its system or broadcast it to its peers; else if it already exists, it will just discard the search query. This
strategy prevents the duplication of the same search query as well as unwanted message flows and helps the systems to
remain stable over message overflow of SER messages. This will considerably reduce the number of messages across the network.

#### If the number of queries (Q) is much larger than the number of nodes (N) (Q >> N)

Since our solution is based on the broadcast of the search query to all the peers it is connected to, when the number of queries
increases, each node gets more search query requests. So this will eventually increase the message count and latency for
...
Since our solution broadcasts the search query to all peers and propagates in the same way to the rest of the network,
when the number of nodes is much higher than the number of queries, search query messages are drastically increased. This will
increase the latency as well as hop count.

To handle this, we can introduce a limit for hops, in other words a time to live, thus after n number of hops,
it won’t propagate. This will limit the propagation of queries beyond a limit and control the number of search query
messages, and also reduce the latency due to fewer messages per node. But it has some issues identifying files/contents if
none of the peers of peers are connected to the file-having nodes to a certain level.

We can apply the **super-peer**, which contains all the resources and only the super-peer can broadcast or do a random walk.
...
We tried 50 sample queries in three selected nodes and got the minimum, maximum, average, and standard deviation for
hops, latency, node degree, and message count per node in different cases (1. all nodes are running, 2. one node is
