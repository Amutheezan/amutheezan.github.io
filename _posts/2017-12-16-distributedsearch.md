...
We tried 50 sample queries in three selected nodes and get the minimum, maximum, average, and standard deviation for hops, latency, node degree, and message per node in each case. Nodes are using UDP sockets for communicating between them. At the end of each case, we gracefully leave one node and continue with the next iteration. And we obtained the following results
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTY5ODkzODA5Myw2NzY5NDQxM119
-->