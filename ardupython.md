...
2.  Install Python 2.7, and install "```pyserial```", "```MYSQLdb```" libraries.
    ...
    3.  Then create the require python program to write to database. for this you need to add two statements "```python import serial```" and "```python import MysqlDb```".
3.  Install Arduino, and Write the code You want to operate. and to read values form sensor , print the data using "```python serial.Println(readData)```" this will acquired by the relevant python file.
...
(I actually referred this code in [4] for my project, hope this will help you)
